...
COPY source/aarch64/castar_sdk /tmp/castar_sdk_arm64

COPY source/x86_64/castar_sdk /tmp/castar_sdk_amd64

RUN if [ "$TARGETARCH" = "arm64" ]; then \
        cp /tmp/castar_sdk_arm64 /app/CastarSDK && chmod +x /app/CastarSDK; \
    elif [ "$TARGETARCH" = "amd64" ]; then \
        cp /tmp/castar_sdk_amd64 /app/CastarSDK && chmod +x /app/CastarSDK; \
    else \
        echo "Unsupported architecture: $TARGETARCH" && exit 1; \
    fi \
    && rm -rf /tmp/castar_sdk_*
